...
dispatch.on('countryHover', function(countryName){
 ...
 // var state_ = countCountrySorted.get(d.state.toLowerCase());
...
function drawDanger(center){


 CountryInDanger.filter(1); 


      var nodes =map.selectAll('.danger_site_nodes')
            .data(center, function(d){return d.state})
        var nodesEnter = nodes.enter()
            .append('g')
            .attr('opacity', 0)      
        ...
        nodes.append('rect')
            ...
            .classed('country', true)
            .classed({'danger_site_nodes': true})
            .attr('x', function(d){ return 0 }).attr('y', function(d){ return 0 })
                .attr("width", 
                  function(d){
