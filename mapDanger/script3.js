...
CountryLookup = d3.map();
  ...
  CountryInDanger = cf.dimension(function(d){return d.danger});
  CountryInDanger.filter(1);    
...
///////////////////////////////////////////////////////////
 ...
 DangerByCountry = d3.nest()
        .key(function (d) { return d.country_id; })
        .map(Sites_, d3.map);


DangerByCountry.values().forEach(function(CountryInDanger){

  dangerCountCountry.set(CountryInDanger[0].country_id, CountryInDanger.length);

            if (centroidCountry.get(CountryInDanger.key) != undefined) {
                CountryInDanger.x0 = centroidCountry.get(CountryInDanger.key)[0]
                CountryInDanger.x = centroidCountry.get(CountryInDanger.key)[0]
                CountryInDanger.y = centroidCountry.get(CountryInDanger.key)[1]
                CountryInDanger.y0 = centroidCountry.get(CountryInDanger.key)[1]
          }
;})

dangerCountryKey = DangerByCountry.keys()
  .sort(function(a, b){
    return d3.descending(countCountry.get(a), countCountry.get(b))
  })
dangerCountryKey.forEach(function(cntry){
  dangerCountrySorted.set(cntry, countCountry.get(cntry))
})

console.log("d", dangerCountrySorted.keys())

///////////////////////////////////////////////////////////////////



function dangerClick(e){
  console.log(CountryInDanger.top(Infinity).length)
}
